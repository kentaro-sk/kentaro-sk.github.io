...
> **このプロジェクトでは、作業の節目ごとに Claude が自動でコミットを実行する。**
> コミットまではユーザーへの個別確認は不要。
> **ただし `git push` の実行前には、必ずユーザーに最終確認を取る**（2026-09-16変更。
> 理由: mainへのpushはGitHub Pagesに即時反映されるため、コミットより一段階リスクが高い
> 操作として明確に分離した）。下記の安全条件も必ず守ること。

### 自動コミットのタイミング

以下の条件を**すべて満たす**ときに、自動でコミットを行う：
...
作業完了後、以下の手順を自動で実行する（ここまではユーザー確認不要）：
...
コミット後、`git push` を実行する **前に** 必ず以下の2点をユーザーに提示し、
明示的な承認（「OK」「プッシュして」等）を得てから `git push origin <ブランチ名>` を実行する。
承認を得るまで `git push` は実行しない。

```
📤 プッシュ確認
　プッシュ先ブランチ: main
　プッシュする作業単位: feat: プロフィールページに研究テーマを追加
　　（未pushコミット一覧: git log origin/main..HEAD --oneline の結果）
このままプッシュしてよいですか？
```

- 「プッシュするブランチ名」: 通常は `main` だが、作業ブランチを切っている場合はそのブランチ名を明記する。
- 「プッシュする作業単位の内容」: 直近1コミットだけとは限らない（複数コミットを溜めてから
  まとめてpushする場合もある）ため、`git log origin/<ブランチ名>..HEAD --oneline` で
  未push分のコミット一覧を取得し、要約して提示する。
...
### 通常のpush確認よりさらに慎重な操作（自動実行しない）

以下の操作は、上記「プッシュ前の確認」の簡易な「OK」だけでは実行せず、
...
**コミット完了時**（push確認を求めるタイミングで）、以下の形式で報告する：
...
📤 プッシュ確認
　プッシュ先ブランチ: main
　プッシュする作業単位: feat: プロフィールページに研究テーマを追加（未pushコミット1件）
このままプッシュしてよいですか？
```

**ユーザーの承認を得てpushが完了したら**、以下の形式で報告する：
