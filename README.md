...
├── DESIGN.md                デザイン方針の正本（配色・書体・余白・コンポーネント・立体感の方針）
...
- **[`DESIGN.md`](./DESIGN.md)**: このサイトのデザインの正本。「Apple風のおしゃれでシンプルなデザイン」を全体方針に、配色（青が基本・金は濃紺面の特別色）、書体（見出しは `.u-display`、HOMEだけ明朝）、余白、角丸、立体感（面の段差・半透明・hoverの浮き上がり）、コンポーネント、ページごとの刷新の進捗を記録しています。AIが見た目を変更するときは、編集の前にこのファイルを読むよう `CLAUDE.md` / `AGENTS.md` で指示し、方針を変えたときは実装と同じ作業の中でこのファイルも更新します
...
このリポジトリは、**対象を分けて**権利を扱っています。詳細は [LICENSE](LICENSE) を参照してください。

| 対象 | 扱い |
|---|---|
| `.claude/`（フック・スキル・エージェント定義）、`scripts/`（検証・補助スクリプト）、`.github/`（CIワークフロー） | **MIT License**。自由に使い、改造してかまいません（著作権表示とライセンス文を残してください）。AI駆動開発の道具立てとして、参考・再利用していただけます |
| サイトの文章・デザイン・写真・イラスト（`index.html`、`pages/`、`css/`、`images/`） | **すべての権利を留保**（© 2026 杉浦健太郎）。無断での転載・再利用はご遠慮ください |
| `js/support.js` | 第三者のビルドツールが出力したバンドルのため、元の作者の条件に従います（上記 MIT の対象外） |
| 上記以外のすべて（`js/` のその他のスクリプト、設定ファイルなど） | MIT の対象外。**すべての権利を留保** |
| `fonts/` のフォント（Parisienne） | SIL Open Font License 1.1（[fonts/OFL-Parisienne.txt](fonts/OFL-Parisienne.txt)） |
